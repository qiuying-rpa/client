    ...
    <template v-for="item, index in props.modelValue" :key="index + refreshKey">
      <div v-if="Object.prototype.toString.call(item) === '[object Object]'"
        ...
        <template v-else>
          ...
          <q-select v-else-if="(item as KwargsFormModelValue).options"
      ...
      <div v-else class="flex items-center" :class="{ 'mt-2': index > 0 }">
        ...
        <div class="ml-2">
          <icon-button icon="i-mdi-plus" @click="addHandler" v-if="index === 0" />
          <icon-button icon="i-mdi-minus" @click="removeHandler(index)" v-else />
        </div>
      </div>
    </template>
  </div>
